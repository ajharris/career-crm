...
profile_industries = Table(
    "profile_industries",
    db.metadata,
    Column("profile_id", ForeignKey("career_profiles.id", ondelete="CASCADE"), primary_key=True),
...
profile_job_families = Table(
    ...
    db.metadata,
    Column("profile_id", ForeignKey("career_profiles.id", ondelete="CASCADE"), primary_key=True),
    Column("job_family_id", ForeignKey("job_families.id", ondelete="CASCADE"), primary_key=True),
)
...
class CareerProfile(db.Model):
    ...
    __table_args__ = (
        ...
        CheckConstraint("applications_per_week_target IS NULL OR applications_per_week_target >= 0"),
    )
    ...
    management_interest: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    technical_leadership_preference: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    ...
    salary_currency: Mapped[str] = mapped_column(String(3), default="CAD", nullable=False)
    ...
    interested_in_networking: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    interested_in_cold_outreach: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    interested_in_recruiter_outreach: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    interested_in_conferences: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    interested_in_government_roles: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    interested_in_academic_roles: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)

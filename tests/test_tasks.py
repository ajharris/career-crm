...
from app.tasks.services import complete_task, create_task, delete_task, list_tasks, reopen_task, update_task
from app.utils.enums import OrganizationType, TaskPriority, TaskStatus, TaskType


def organization():
    return create_organization(name="UHN", organization_type=OrganizationType.HOSPITAL.value, priority=4)


def values(**extra):
    data = {"title": "Send follow-up", "task_type": TaskType.FOLLOW_UP.value,
            "priority": TaskPriority.MEDIUM.value, "status": TaskStatus.OPEN.value,
            "due_date": date.today() + timedelta(days=2)}
    data.update(extra); return data


def form_data(**extra):
    data = values(); data["due_date"] = data["due_date"].isoformat(); data.update(extra); return data
...
def test_relationship_and_set_null_history(app):
    org = organization(); task = create_task(**values(organization_id=org.id)); task_id = task.id
    db.session.delete(org); db.session.commit()
...
def test_services_create_update_delete(app):
    ...
    task_id = task.id; delete_task(task); assert db.session.get(Task, task_id) is None


def test_complete_reopen_and_activity(app):
    org = organization(); task = create_task(**values(organization_id=org.id))
    complete_task(task)
    ...
    reopen_task(task); assert task.status is TaskStatus.OPEN and task.completed_at is None
...
def test_pagination(app):
    for index in range(26): create_task(**values(title=f"Task {index}"))
...
def test_crud_routes(client):
    ...
    response = client.post(f"/tasks/{task.id}/edit", data=form_data(title="Edited"), follow_redirects=True)
...
def test_activity_followup_prefills_without_creating(client, app):
    org = organization()
    activity = Activity(organization_id=org.id, activity_type="email", direction="outbound",
                        occurred_at=datetime.now(), subject="Check in", follow_up_needed=True)
